"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.ResetPasswordUserDto = exports.ResetPasswordEmailDto = void 0;
const openapi = require("@nestjs/swagger");
const class_validator_1 = require("class-validator");
class ResetPasswordEmailDto {
    static _OPENAPI_METADATA_FACTORY() {
        return { newPassword: { required: true, type: () => String } };
    }
}
exports.ResetPasswordEmailDto = ResetPasswordEmailDto;
__decorate([
    (0, class_validator_1.IsString)(),
    (0, class_validator_1.Length)(6, 16),
    __metadata("design:type", String)
], ResetPasswordEmailDto.prototype, "newPassword", void 0);
class ResetPasswordUserDto {
    static _OPENAPI_METADATA_FACTORY() {
        return { password: { required: false, type: () => String }, newPassword: { required: true, type: () => String } };
    }
}
exports.ResetPasswordUserDto = ResetPasswordUserDto;
__decorate([
    (0, class_validator_1.IsString)(),
    (0, class_validator_1.Length)(6, 16),
    __metadata("design:type", String)
], ResetPasswordUserDto.prototype, "password", void 0);
__decorate([
    (0, class_validator_1.IsString)(),
    (0, class_validator_1.Length)(6, 16),
    __metadata("design:type", String)
], ResetPasswordUserDto.prototype, "newPassword", void 0);
//# sourceMappingURL=reset-password.dto.js.map